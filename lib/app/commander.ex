defmodule App.Commander do
  ...
  def generate_callback_query_matcher(function) do
    quote do
      def do_match_message(%{callback_query: callback_query} = var!(message))
  ...
  defmacro answer_callback_query(options \\ []) do
    quote bind_quoted: [options: options] do
      Nadia.answer_callback_query var!(message).callback_query.id, options
    end
  end

  defmacro answer_inline_query(results, options \\ []) do
    quote bind_quoted: [results: results, options: options] do
      Nadia.answer_inline_query var!(message).inline_query.id, results, options
    end
  end
  ...
  defmacro send_audio(audio, options \\ []) do
    quote bind_quoted: [audio: audio, options: options] do
      case var!(message) do
        ...
        message ->
          Nadia.send_audio var!(message).message.chat.id, audio, options
      end
    end
  end
  ...
  defmacro send_chat_action(action) do
    quote bind_quoted: [action: action] do
      case var!(message) do
        ...
        message ->
          Nadia.send_chat_action var!(message).message.chat.id, action
      end
    end
  end
      ...
      case var!(message) do
        ...
        %{callback_query: callback_query} when not is_nil(callback_query) ->
          Nadia.send_contact callback_query.message.chat.id, phone_number,
                             first_name, options
        message ->
          Nadia.send_contact var!(message).message.chat.id, phone_number,
                             first_name, options
      end
    end
  end
  ...
  defmacro send_document(document, options \\ []) do
    quote bind_quoted: [document: document, options: options] do
      case var!(message) do
        ...
        message ->
          Nadia.send_document var!(message).message.chat.id, document, options
      end
    end
  end
      ...
      case var!(message) do
        ...
        %{callback_query: callback_query} when not is_nil(callback_query) ->
          Nadia.send_location callback_query.message.chat.id, latitude,
                              longitude, options
        message ->
          Nadia.send_location var!(message).message.chat.id, latitude,
                              longitude, options
      end
    end
  end
  ...
  defmacro send_photo(photo, options \\ []) do
    quote bind_quoted: [photo: photo, options: options] do
      case var!(message) do
        ...
        message ->
          Nadia.send_photo var!(message).message.chat.id, photo, options
      end
    end
  end
  ...
  defmacro send_sticker(sticker, options \\ []) do
    quote bind_quoted: [sticker: sticker, options: options] do
      case var!(message) do
        ...
        message ->
          Nadia.send_sticker var!(message).message.chat.id, sticker, options
      end
    end
  end
  ...
  defmacro send_venue(latitude, longitude, title, address, options \\ []) do
    quote bind_quoted: [latitude: latitude, longitude: longitude,
      ...
      case var!(message) do
        ...
        message ->
          Nadia.send_venue var!(message).message.chat.id, latitude, longitude,
                           title, address, options
      end
    end
  end
  ...
  defmacro send_video(video, options \\ []) do
    quote bind_quoted: [video: video, options: options] do
      case var!(message) do
        ...
        message ->
          Nadia.send_video var!(message).message.chat.id, video, options
      end
    end
  end
  ...
  defmacro send_voice(voice, options \\ []) do
    quote bind_quoted: [voice: voice, options: options] do
      case var!(message) do
        ...
        message ->
          Nadia.send_voice var!(message).message.chat.id, voice, options
      end
    end
  end
  ...
  defmacro forward_message(chat_id) do
    quote bind_quoted: [chat_id: chat_id] do
      case var!(message) do
        ...
        message ->
          Nadia.forward_message chat_id, var!(message).message.chat.id,
  ...
  defmacro get_chat do
    quote do
      case var!(message) do
        ...
        message ->
          Nadia.get_chat var!(message).message.chat.id
      end
    end
  end
  ...
  defmacro get_chat_administrators do
    quote do
      case var!(message) do
        ...
        message ->
          Nadia.get_chat_administrators var!(message).message.chat.id
      end
    end
  end
  ...
  defmacro get_chat_member(user_id) do
    quote bind_quoted: [user_id: user_id] do
      case var!(message) do
        ...
        message ->
          Nadia.get_chat_member var!(message).message.chat.id, user_id
      end
    end
  end
  ...
  defmacro get_chat_members_count do
    quote do
      case var!(message) do
        ...
        message ->
          Nadia.get_chat_members_count var!(message).message.chat.id
      end
    end
  end
  ...
  defmacro kick_chat_member(user_id) do
    quote bind_quoted: [user_id: user_id] do
      case var!(message) do
        ...
        message ->
          Nadia.kick_chat_member var!(message).message.chat.id, user_id
      end
    end
  end
  ...
  defmacro leave_chat do
    quote do
      case var!(message) do
        ...
        message ->
          Nadia.leave_chat var!(message).message.chat.id
      end
    end
  end
  ...
  defmacro unban_chat_member(user_id) do
    quote bind_quoted: [user_id: user_id] do
      case var!(message) do
        ...
        message ->
          Nadia.unban_chat_member var!(message).message.chat.id, user_id
      end
    end
  end
end
